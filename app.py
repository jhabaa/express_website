...
def fill_database():
    cursor = mysql.new_cursor( dictionary = True)
    # get stores
    cursor.execute("SELECT * FROM expressdc.store")
    stores = cursor.fetchall()
    cursor.reset()
    # get services
    cursor.execute("SELECT * FROM expressdc.service")
    services = cursor.fetchall()
    #fill the store_has_service table
    for service in services:
        for store in stores:
            cursor.reset()
            print((f"INSERT INTO `expressdc`.`store_has_service` (`store_name`, `service_name`,`cost`) VALUES ('{store['name']}', '{service['name']}', '5.00');"))
            cursor.execute(f"INSERT INTO `expressdc`.`store_has_service` (`store_name`, `service_name`,`cost`) VALUES ('{store['name']}', '{service['name']}', '5.00');")
            mysql.connection.commit()
