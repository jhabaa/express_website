...
def GetData(filetype:str  ,name: str, where = None):
    ...
    if where:
        ...
        if "." not in where:
            return []
        values = where.split(".")
        com = f"SELECT * FROM {app.config['MYSQL_DB']}.{name} WHERE {values[0]} = '{values[1]}'"
        print(com)
        cur.execute(com)
    else:
...
def pricing(lang = 'fr', subpage = None):
    ...
    categories = GetData('full','category')
    services_in_store = GetData('full','store_has_service')
    ...
    return render_template('pricing.html', lang = session['lang'], services = None if len(available_services) <= 0 else available_services, categories = categories, services_in_store = services_in_store, dictionary = dictionary[lang], other_categories = others_categories )
